...
void pay_money(pPlayers player, Cell *cells, pPlayers now_user_for_pay_money,
  ...
  if ((*cells).owner != 0 &&
      (*cells).owner != (*now_user_for_pay_money).number &&
      (*cells).kind != 4) {
    if ((*now_user_for_pay_money).god > 0) {
      ...
      // 现有钱数少于支付钱数
        ...
        for (int i = 0; i <= 3; i++) {
          // 如果当前位置有人并且这个人不是当前角色
          if (player[i].position == (*now_user_for_pay_money).position &&
              player[i].number != (*now_user_for_pay_money).number) {
            // 根据当前位置的人重置地图符号
        ...
        for (int i = 0; i <= 3; i++) {
          if (player[i].isPlaying == 1)
            playing_num++;
          if (player[i].isBankrupt == 1)
            dead_num++;
          if (player[i].isPlaying == 1 && player[i].isBankrupt == 0)
            final_role_num = player[i].number;
          if (player[i].isPlaying == 1 && player[i].isBankrupt == 1)
            clean_body(map, &player[i]);
        }
